...
ข้อมูลนำเข้า: คุณจะได้รับภาพ (image) ของเอกสาร PDF ซึ่งมีข้อมูลในรูปแบบตารางอย่างชัดเจน โปรดทราบว่าข้อความที่สกัดมาอาจมีข้อผิดพลาดจากการรู้จำอักขระด้วยแสง (OCR errors) ทำให้ชื่อหรือข้อมูลอื่นๆ อาจมีการสะกดผิดเพี้ยนไปจากต้นฉบับ และบางครั้งข้อมูลคอลัมน์อาจไม่ตรงแถวกัน ทำให้ "เลขหมายประจำบ้าน" อาจไม่ตรงกับ "ชื่อตัว - ชื่อสกุล" ในบางรายการ
วัตถุประสงค์: วิเคราะห์ข้อมูลตารางในภาพที่ให้มา และดึงเฉพาะข้อมูลจากคอลัมน์ต่อไปนี้:
"เลขหมายประจำบ้าน" (House Number)
"ชื่อตัว - ชื่อสกุล" (Full Name)
...
พยายามถอดความจากต้นฉบับ: หากชื่อในคอลัมน์ "ชื่อตัว - ชื่อสกุล" มีการสะกดผิดเพี้ยนเล็กน้อย 
(เช่น ตัวอักษรสลับกัน, ตกหล่น) ให้คาดเดาได้เล็กน้อยและถอดความเป็นชื่อที่ใกล้เคียงกับชื่อคน โดยอ้างอิงจากรูปแบบชื่อทั่วไป และบริบทของข้อมูลในรายการนั้นๆ
ห้ามพยายามคาดเดาและถอดความเป็นชื่อผิดจากต้นฉบับมากจนเป็นคนละคน
...
ห้ามพยายามจับคู่ผิดพลาดจนเป็นคนละคน และให้ค่อยๆ เช็คจำนวนให้ตรงกับรูปภาพให้มากที่สุด เช่น มี 22 คน ต้องมี 22 คนในผลลัพธ์
...
รูปแบบผลลัพธ์ที่ต้องการ (JSON): โปรดส่งคืนผลลัพธ์เป็น JSON object ที่มีโครงสร้างดังนี้: { "records": [ { "${nameLabel}": "สมชาย ใจดี", "${addressLabel}": "123/45" }, { "${nameLabel}": "สุภาพร สุขใจ" }, { "${nameLabel}": "มานะ พัฒนา", "${addressLabel}": "50/2" }, { "${nameLabel}": "นิดหน่อย รักเรียน" }, { "${nameLabel}": "ธรรา สุรพร" }, { "${nameLabel}": "[ชื่อไม่ชัดเจน]" }, { "${nameLabel}": "วิภาวดี มีสุข" } ] }
กฎสำคัญ:
    ...
    cors(req, res, async () => {
      ...
      try {
          ...
          if (isPdf) {
            ...
            if (req.body.pageRange && Array.isArray(req.body.pageRange) && req.body.pageRange.length > 0) {
              ...
              if (pageRange.length > 0) {
                ...
                console.log(`📄 [SMART_OCR_VISION] Using pageRange: [${pageRange.join(', ')}]`);
              }
            } else {
        ...
        for (let i = 0; i < pagesToProcess.length; i++) {
          ...
          console.log(`[SMART_OCR_VISION] 📤 Sending page ${pageNumber}/${totalPages} to Gemini...`, { reqId, userId, deviceId, sessionId, pageNumber, totalPages, progress: pageProgress });
